...
const MemberDetails: React.FC = () => {
  ...
  if (loading) return <div className="text-center py-12">Loading member details...</div > ;
  ...
  const billColumns: Column<MembershipHistory>[] = [
    ...
    { key: 'debt', header: 'Debt',    render: (row) => row.debt > 0 ? <span className="text-red-500 font-medium">${row.debt}</span> : '$0' },
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center space-x-4">
        <Button variant="secondary" onClick={() => navigate('/members')} className="p-2 rounded-full">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <Card className="flex flex-col items-center text-center">
            <div className="w-24 h-24 bg-blue-100 rounded-full flex items-center justify-center text-blue-600 mb-4">
              {member.profilePicture ? (
                <img src={member.profilePicture} className="w-full h-full object-cover rounded-full" />
              ) : (
                <User size={48} />
              )}
            ...
            <div className="w-full mt-6 space-y-4 text-left border-t border-gray-100 pt-6">
              ...
              <div className="flex items-center text-gray-700">
                <Calendar className="w-5 h-5 mr-3 text-gray-400" /> Registered: {member.startDate}
           
              </div>
              
              <div className="flex items-center text-gray-700">
                <Activity className="w-5 h-5 mr-3 text-gray-400" /> Ends: {member.endDate}
              </div>
              <div className="flex items-center text-red-600 font-medium pt-2 border-t border-gray-50">
                <CreditCard className="w-5 h-5 mr-3" /> Total Debt: ${member.debt}
              </div>
              <div className="flex items-center space-x-2">
              <Button onClick={() => setIsEditModalOpen(true)} className="bg-black text-white">
                Edit
              </Button>

              <Button onClick={handleDeleteMember} className="bg-red-500 text-white">Delete</Button>
              </div>
            </div>
          </Card>
        </div>
        ...
        <div className="lg:col-span-2 space-y-6">
          <Card>
            ...
            <form onSubmit={handleAddMembership} className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="col-span-1 mb-4">
                ...
                <Input label="Duration (Days)" type="number" value={selectedPackageId ? String(packages.find(p => String(p.id) === selectedPackageId)?.durationInDays ?? '') : ''} disabled />
              </div>
              ...
              <div className="col-span-1">
                <Input label="Debt ($)" type="number" value={debt} onChange={e => setDebt(e.target.value)} />
              </div>
              <div className="col-span-1">
          ...
          <Card>
            ...
            <Table data={memberships} columns={billColumns} keyExtractor={(row)=>row.id} />
          </Card>
        </div>
      </div>
